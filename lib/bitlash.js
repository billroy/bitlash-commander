...
Bitlash.prototype = {
	...
	seriallistener: function(data) {
		...
		for (;;) {		// peel off full lines of input containing JSON
			...
			var first_line = this.instream.slice(0, m.index + 1);
			this.instream = this.instream.slice(m.index + 1);

console.log('Line:', first_line, this.instream);

			// detect JSON update sent by Bitlash
			if (first_line.match(this.prompt)) {
				this.sendReply();
				first_line = first_line.slice(this.promptlength);
			}
			if (first_line.charAt(0) == '{') {
				console.log('JSON upstream:', JSON.parse(first_line));
				// todo: send to updater
			}
...
console.log('sendreply:', this.reply);
	...
	exec: function(command, callback) {
		if (this.callback) console.log('ERROR: bitlash.exec() during busy');
